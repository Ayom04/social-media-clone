...
const registerUser = async(req,res) => {
    ...
    const {surname,othernames,phone,email_address,user_name ,gender,date_of_birth,
            occupation,password} = req.body;
    try {
...
const verifyUser = async (req, res)=>{
    ...
    try {
        ...
        await models.Users.update({is_verified : true},{ 
            where:{
                email_address
            }
        })
...
const resendOtp = async (req, res) => {
    ...
    try {
        ...
        sendEmail(email_address, 'Resend OTP ', `Hi ${checkIfuserExists.othernames}, Your new OTP is ${_otp}. Kindly note that this OTP expires in 5 minutes.`,)
        res.status(200).json({
            status: true,
